...
class MagicFormulaLateral:
	def __init__(self, filename, data_path, epsilon=None, zeta=None, config_path=None):
		config = load_settings(config_path)["tire_model"]
		self.param_tires = {}
		for ele in filename:
			self.param_tires[ele] = self.load_params(ele, data_path)
		self.epsilon = config["epsilon"] if epsilon is None else epsilon
		self.zeta = config["zeta"] if zeta is None else zeta
	...
	def load_params(self, filename, data_path):
		...
		matches = sorted(path for path in data_dir.iterdir() if filename in path.name)
		if not matches:
			raise FileNotFoundError(f"No tire parameter file containing {filename!r} in {data_dir}.")
		with matches[0].open('r', encoding='utf-8') as stream:
			try:
				parsed_yaml = yaml.safe_load(stream)
			except yaml.YAMLError as exc:
				raise ValueError(f"Could not parse tire parameter file {matches[0]}.") from exc
		return parsed_yaml
